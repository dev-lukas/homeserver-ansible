...
- **Auth + TLS terminate in the registry itself** (bcrypt htpasswd from
  `secret_reverse_proxy_auth_basic_users.registry`, self-signed cert for the
...
- **Credentials**: password shared with CI (`REGISTRY_PASSWORD` secret) and
  the VPS (`secret_firephenix_registry_password`). The local retention script
  reads `/etc/registry/retention-credentials`
  (from `secret_registry_retention_credentials`).
