# Vendored from community.general main @ 99df4d8bebe7 (2026-09-20): the released 13.4.0
# plugin probes the session with 'pass-cli test', which pass-cli >= 2.3 removed.
# Local patch on top: custom items (sections[].section_fields) are parsed too.
# Delete this file and switch secrets.yml back to community.general.proton_pass
# once a release with the 'pass-cli info' probe is out.
    ...
    if isinstance(type_dict, dict):
        for type_key, type_data in type_dict.items():
            if isinstance(type_data, dict):
                ...
                # pass-cli >= 2.3 emits custom-section fields as sections[].section_fields
                # (local patch, not upstream yet).
                for section in type_data.get("sections", []):
                    fields.update(_extract_extra_fields(section.get("section_fields", [])))
                    fields.update(_extract_extra_fields(section.get("extra_fields", [])))
